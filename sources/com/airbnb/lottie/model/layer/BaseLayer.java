...
public abstract class BaseLayer implements DrawingContent, BaseKeyframeAnimation.AnimationListener, KeyPathElement {
    ...
    private void applyMasks(Canvas canvas, Matrix matrix2) {
        ...
        for (int i2 = 0; i2 < this.mask.getMasks().size(); i2++) {
            Mask mask2 = this.mask.getMasks().get(i2);
            BaseKeyframeAnimation baseKeyframeAnimation = this.mask.getMaskAnimations().get(i2);
            BaseKeyframeAnimation baseKeyframeAnimation2 = this.mask.getOpacityAnimations().get(i2);
    ...
    public static BaseLayer forModel(Layer layer, LottieDrawable lottieDrawable2, LottieComposition lottieComposition) {
        ...
        StringBuilder y = a.y("Unknown layer type ");
        y.append(layer.getLayerType());
    ...
    public void draw(Canvas canvas, Matrix matrix2, int i2) {
        ...
        int intValue = (int) ((((((float) i2) / 255.0f) * ((float) (this.transform.getOpacity() == null ? 100 : this.transform.getOpacity().getValue().intValue()))) / 100.0f) * 255.0f);
    ...
    public abstract void drawLayer(Canvas canvas, Matrix matrix2, int i2);
    ...
    public void setProgress(@FloatRange(from = 0.0d, to = 1.0d) float f) {
        ...
        if (this.mask != null) {
            for (int i2 = 0; i2 < this.mask.getMaskAnimations().size(); i2++) {
                this.mask.getMaskAnimations().get(i2).setProgress(f);
            }
        }
        ...
        for (int i3 = 0; i3 < this.animations.size(); i3++) {
            this.animations.get(i3).setProgress(f);
        }
    }
}
