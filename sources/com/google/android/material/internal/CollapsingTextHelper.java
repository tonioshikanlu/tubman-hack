...
public final class CollapsingTextHelper {
    ...
    private void calculateOffsets(float f) {
        ...
        if (this.collapsedTextColor != this.expandedTextColor) {
            ...
            i2 = blendColors(getCurrentExpandedTextColor(), getCurrentCollapsedTextColor(), f);
        } else {
            textPaint2 = this.textPaint;
            i2 = getCurrentCollapsedTextColor();
        }
    ...
    private void calculateUsingTextSize(float f) {
        ...
        if (this.text != null) {
            ...
            if (this.textToDraw == null || z) {
                ...
                StaticLayout createStaticLayout = createStaticLayout(i2, width, this.isRtl);
    ...
    private StaticLayout createStaticLayout(int i2, float f, boolean z) {
        StaticLayout staticLayout;
        try {
            staticLayout = StaticLayoutBuilderCompat.obtain(this.text, this.textPaint, (int) f).setEllipsize(TextUtils.TruncateAt.END).setIsRtl(z).setAlignment(Layout.Alignment.ALIGN_NORMAL).setIncludePad(false).setMaxLines(i2).build();
    ...
    private float getCollapsedTextLeftBound(int i2, int i3) {
        return (i3 == 17 || (i3 & 7) == 1) ? (((float) i2) / 2.0f) - (calculateCollapsedTextWidth() / 2.0f) : ((i3 & GravityCompat.END) == 8388613 || (i3 & 5) == 5) ? this.isRtl ? (float) this.collapsedBounds.left : ((float) this.collapsedBounds.right) - calculateCollapsedTextWidth() : this.isRtl ? ((float) this.collapsedBounds.right) - calculateCollapsedTextWidth() : (float) this.collapsedBounds.left;
    }

    private float getCollapsedTextRightBound(@NonNull RectF rectF, int i2, int i3) {
        if (i3 == 17 || (i3 & 7) == 1) {
            return (calculateCollapsedTextWidth() / 2.0f) + (((float) i2) / 2.0f);
