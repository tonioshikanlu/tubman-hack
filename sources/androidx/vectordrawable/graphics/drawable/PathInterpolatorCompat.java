...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            ...
            if (((double) Math.abs(this.mX[0])) <= 1.0E-5d && ((double) Math.abs(this.mY[0])) <= 1.0E-5d) {
                int i4 = min - 1;
                if (((double) Math.abs(this.mX[i4] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.mY[i4] - 1.0f)) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i5 = 0;
                    while (i2 < min) {
                        float[] fArr2 = this.mX;
            ...
            StringBuilder y = a.y("The Path must start at (0,0) and end at (1,1) start: ");
            y.append(this.mX[0]);
            y.append(",");
            y.append(this.mY[0]);
            y.append(" end:");
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            throw new InflateException(a.m("The path is null, which is created from ", namedString));
