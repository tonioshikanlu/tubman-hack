...
public class SuggestionsAdapter extends ResourceCursorAdapter implements View.OnClickListener {
    ...
    private Drawable getActivityIcon(ComponentName componentName) {
        ...
        try {
            ...
            StringBuilder z = a.z("Invalid icon resource ", iconResource, " for ");
            z.append(componentName.flattenToShortString());
            str = z.toString();
            Log.w(LOG_TAG, str);
            return null;
                ...
                if (pathSegments != null) {
                    ...
                    if (size == 1) {
                        try {
                            ...
                            throw new FileNotFoundException(a.k("Single path segment is not a resource ID: ", uri));
                        }
                    } else if (size == 2) {
                        i2 = resourcesForApplication.getIdentifier(pathSegments.get(1), pathSegments.get(0), authority);
                    } else {
                        throw new FileNotFoundException(a.k("More than two path segments: ", uri));
                    }
                    if (i2 != 0) {
                        return resourcesForApplication.getDrawable(i2);
                    }
                    throw new FileNotFoundException(a.k("No resource found for: ", uri));
                }
                throw new FileNotFoundException(a.k("No path: ", uri));
            } catch (PackageManager.NameNotFoundException unused2) {
                throw new FileNotFoundException(a.k("No package found for authority: ", uri));
            }
        } else {
            throw new FileNotFoundException(a.k("No authority: ", uri));
        }
    }
